...
async def get_chat_sessions(db = Depends(get_db)):
    """
    Retrieves all past chat sessions so the user can review their conversation history.
    """
    chat_sessions = db.query(ChatSession).order_by(ChatSession.created_at.desc()).all()
...
async def get_documents(db = Depends(get_db)):
    ...
    """
    documents = db.query(UploadedFile).order_by(UploadedFile.created_at.desc()).all()
